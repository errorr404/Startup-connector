...
router.post('/api/register',(req,res)=>{
    ...
    StartupOwner.findOne({email:req.body.email}).then(user_res=>{
        console.log('hello-->',user_res)
        if(user_res){
        ...
        user.save()
            ...
            }).catch(err=>res.status(500).send({"message":"error while registering!"}))
    }).catch(err=>res.status(500).send({"message":"error while registering!"}))
})
...
router.post('/login',(req,res)=>{
    ...
    User.findOne({email:email})
        .then(user_res=>{
            if(user_res){
                if(bcrypt.compareSync(email,user_res.password)){
